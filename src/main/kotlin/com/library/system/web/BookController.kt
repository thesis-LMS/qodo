...
class BookController(
    ...
    ): ResponseEntity<Book> = ResponseEntity.status(HttpStatus.CREATED).body(bookService.addBook(book)) // Ensure 201

    @GetMapping("/{id}")
    fun getBook(
        @PathVariable id: UUID,
    ): ResponseEntity<Book> = ResponseEntity.ok(bookService.getBookById(id))
    ...
    fun updateBook(
        @PathVariable id: UUID,
        @RequestBody book: Book,
    ): ResponseEntity<Book> = ResponseEntity.ok(bookService.updateBook(id, book))
    ...
    fun returnBook(
        @PathVariable id: UUID,
    ): ResponseEntity<Book> = ResponseEntity.ok(bookService.returnBook(id))
}
